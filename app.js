...
app.get('/heroes/detalle/:id', (req, res) => {
	...
	if (heroe[0]==undefined){
		...
		res.send(`Hola, mi nombre es ${heroe[0].nombre} y soy
		${heroe[0].profesion}.`)
	}
});
...
app.get('/heroes/:bio/:id/:ok?', (req, res) => {
	...
	if (heroe[0]==undefined){
		...
		res.send(`Héroe: ${heroe[0].nombre}.<br/>Reseña: ${heroe[0].resenia}.`)
		//El <br/> se utiliza para hacer un salto de linea ya que '\' no me funciona
	}else{
		res.send(`${heroe[0].nombre} dice:  Lamento que no desees saber más de mi :(`)
	}
}
...
res.send(`Autor: Lezcano, Sebastian Emanuel :)`)
})
